            ...
            if let sel = localSelection {
                if selectedLayoutName == "Default" {
                    DefaultListView(
                        ...
                        categoryIcon: sel.icon,
                        categoryColour: sel.colour,   // 👈 UInt
                        onSave: { _ in }
                    )
                    ...
                    // fallback if no layout picked yet
                    EmptyView()
                }
            } else {
                // safety fallback if presented without a selection
                EmptyView()
            }
        }
